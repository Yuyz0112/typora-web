...
| reference link def `[id]: url` | 🟡 | `ref-def` | Live editor input: paragraph `[label]:` triggers a draft (gray-italic on `[`, `]:`), Enter on a full `[label]: url ["title"]` line commits to a `link_def` atom block. The block serialises to `[label]: url` on save. **Caveat**: md-it consumes `[id]: url` lines silently on parse, so reloading a saved file drops the def node (the inline `[t][id]` references still resolve to inline links). Full preservation needs a custom md-it block rule + parserPostProcess (phase 2). |
| math block `$$…$$` | ⏸️ | — | Deferred. Will be done as an opt-in plugin alongside inline math (KaTeX renderer). |
| table `\| a \| b \|` | ✅ | `table` | Parse / serialize / display + alignment (`:---`, `:---:`, `---:`). Inline marks in cells preserved. Live entry: paragraph `\|c1\|c2\|...\|<Enter>` (≥ 2 cells) commits to table. Tab/Shift-Tab cell nav (boundary cells consume but don't move). Floating toolbar on focus: resize popup (田字格 10×10 hover-grid + numeric R×C), per-column left/center/right align, trash. Full-width display. |
