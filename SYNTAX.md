...
| table `\| a \| b \|` | 🟡 | `table` | Parse / serialize / display + alignment (`:---`, `:---:`, `---:`). Inline marks in cells (em / strong / code / strike / etc.) preserved. Cell content is `inline*` (single line). Live entry: paragraph text `\|c1\|c2\|...\|` (≥ 2 cells, leading + trailing pipes) + Enter commits to table; cursor lands in first body cell. **Phase 2 pending**: Tab/Shift-Tab cell nav, row/col add-delete UI. |
